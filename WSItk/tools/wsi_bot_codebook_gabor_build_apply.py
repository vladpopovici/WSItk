...
def worker(img_name, desc, wnd_size, idx):
    ...
    bag = grow_bag_from_new_image(im_h, desc, (wnd_size, wnd_size), 1000000,
                                  sampling_strategy='sliding', it_step=(wnd_size,wnd_size), discard_empty=True)

    # save the bag:
    with ModelPersistence('.'.join(img_name.split('.')[:-1])+'_bag_gabor_l0.pkl', 'c', format='pickle') as d:
        d['bag_l0'] = bag
...
def main():
    ...
    vq = MiniBatchKMeans(n_clusters=args.codebook_size, random_state=rng, n_init=10,
                         batch_size=5000, compute_labels=True, verbose=False)   # vector quantizer
